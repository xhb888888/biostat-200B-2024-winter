...
## Investigators hypothesize that xray will be significantly related to risk after controlling for beds, nurses and svcs. What model should you fit to test this hypothesis? Fit the model and report the results. Do the results support the hypothesis or not? Provide an interpretation of each of the regression coefficients, using appropriate units. Also report the partial correlation of each predictor with risk. Conduct a joint test of whether beds and nurses contribute to explaining variation in risk after controlling for svcs and xray.
...
The result supports the hypothesis that xray is significantly related to risk since the p-value in partial F test for xray is less than 0.0001. Here are interpretations for each regression coefficient:
...
The joint test of whether beds and nurses contribute to explaining variation in risk after controlling for svcs and xray is:

$H_0: \beta_{beds}=\beta_{nurses}=0$

$H_A: \beta_{beds},\beta_{nurses}$ are not all equal to zero

![](images/WeChateed13dc2229fe97b1ab62029a11988f4.png){width="371"}

Conclusion: Since the pvalue is smaller than 0.05, we reject the null hypothesis. There is evidence that at least one of the variable is significant in associating with the expected risk for hospital, after controlling for svcs and xray.
